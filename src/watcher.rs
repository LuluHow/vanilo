...
pub fn watch(build_lock: Arc<Mutex<()>>) -> Result<(), String> {
    ...
    loop {
        ...
        // Drain spurious events generated by reading watched dirs during build
        while rx.try_recv().is_ok() {}

        // If source files were genuinely modified during the build, rebuild
        if any_source_modified_since(build_start) {
            needs_rebuild = true;
        }
    }
}

/// Returns true if any watched source file has mtime >= `since`.
fn any_source_modified_since(since: SystemTime) -> bool {
    for dir in WATCH_DIRS {
        let path = Path::new(dir);
        if path.is_dir() && dir_modified_since(path, since) {
            return true;
        }
    }
    for file in WATCH_FILES {
        if file_modified_since(Path::new(file), since) {
            return true;
        }
    }
    false
...
fn dir_modified_since(dir: &Path, since: SystemTime) -> bool {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_dir() {
            if dir_modified_since(&path, since) {
                return true;
            }
        } else if file_modified_since(&path, since) {
            return true;
        }
    }
    false
...
mod tests {
    ...
    #[test]
    fn modified_since_detects_new_file() {
        let dir = std::env::temp_dir().join(format!("vanilo_test_mtime_{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();

        let before = SystemTime::now();
        // Small sleep so the mtime is strictly after `before`
        thread::sleep(Duration::from_millis(50));
        fs::write(dir.join("new.html"), "hello").unwrap();

        assert!(dir_modified_since(&dir, before));
        // File written before the threshold should not match
        let future = SystemTime::now() + Duration::from_secs(2);
        assert!(!dir_modified_since(&dir, future));

        let _ = fs::remove_dir_all(&dir);
    }

    #[test]
    fn modified_since_ignores_old_files() {
        let dir = std::env::temp_dir().join(format!("vanilo_test_mtime_old_{}", std::process::id()));
        let _ = fs::remove_dir_all(&dir);
        fs::create_dir_all(&dir).unwrap();
        fs::write(dir.join("old.html"), "old").unwrap();

        thread::sleep(Duration::from_millis(50));
        let after = SystemTime::now();

        assert!(!dir_modified_since(&dir, after));
